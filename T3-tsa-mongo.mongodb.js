...
//Comments for your marker: In task 3(d), a db.find statement has been added after the updateMany function to show change in the state of the database
//                          In task 3(e), db.find statements are added both before and after the updateOne function to show before and after states of the database
...
db.townResortDetails.insertMany([
...
// ENSURE that your query is formatted and has a semicolon
// (;) at the end of this answer
db.townResortDetails.find({ "$and": [{"avg_temperature.summer": {"$gt": 25}}, 
...
// ENSURE that your query is formatted and has a semicolon
// (;) at the end of this answer
db.townResortDetails.updateMany({"resorts.company_name": {"$eq": "Tropical Dream"}}, 
                                {"$set": {"resorts.$.company_name": "Tropical Heaven"}});

// this db.find command looks for documents which have resort company name of "Tropical Dream"
// -- it returns an empty list since all "Tropical Dream" company names have been replaced with "Tropical Heaven"
db.townResortDetails.find({"resorts.company_name": {"$eq": "Tropical Dream"}});
...
//state of Mount Isa town before addition of new resort
db.townResortDetails.find({"$and": [{"name": "Mount Isa, QLD"},
                                    {"location.latitude": -20.7256}, 
                                    {"location.longitude": 139.4927}]});

//adding new Amazing Resort of company Australia Experience to town name Mount Isa, QLD
db.townResortDetails.updateOne({"$and": [{"name": "Mount Isa, QLD"},
                                        {"location.latitude": -20.7256}, 
                                        {"location.longitude": 139.4927}]},
                                {"$push": {"resorts": {"id": 10, 
                                                       "name": "Amazing Resort", 
                                                       "address": "73 Mount Isa District", 
                                                       "phone": "0091210671", 
                                                       "year_built": "2019", 
                                                       "company_name": "Australia Experience"}}});

//manually setting no. of resorts to 2
db.townResortDetails.updateOne({"$and": [{"name": "Mount Isa, QLD"},
                                        {"location.latitude": -20.7256}, 
                                        {"location.longitude": 139.4927}]},
                                {"$set": {"no_of_resorts": 2}});
...
//state of Mount Isa town after addition of new resort Amazing Resort
db.townResortDetails.find({"$and": [{"name": "Mount Isa, QLD"},
                                    {"location.latitude": -20.7256}, 
                                    {"location.longitude": 139.4927}]});
...
//final complete state of db
db.townResortDetails.find();
